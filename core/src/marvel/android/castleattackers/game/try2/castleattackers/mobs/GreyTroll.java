...
public class GreyTroll extends GameLiving {
	...
	public GreyTroll(float x, float y, Species species, int mobLevel, int lane) {
		...
		lifeTime = 0;
			...
			isCaster = false;
			isRange = false;
			attackRange = 35; // Der wird am Ende denke mal in Pixeln gez�hlt
								// werden
			attackSpeed = 1.2f; // 0.4f = Soll 0.4x/sec zuhaun
			maxLivingTime = 75; // Soll : In Sekunden
			this.species = World.Species.greyTroll;
	...
	public TextureRegion getAnimation() {
		switch (state) {
		case attack:
				return (TextureRegion) Assets.gta.getKeyFrame(stateTime, true);
				
		case walk:
				return (TextureRegion) Assets.gtw.getKeyFrame(stateTime, true);
			
		case hit:
				return (TextureRegion) Assets.gth.getKeyFrame(stateTime, true);
			
		case die:
				return (TextureRegion) Assets.gtd.getKeyFrame(stateTime, false);
		}
		return null;
	}
