...
public class IceDragon extends GameLiving {
	...
	public IceDragon(float x, float y,
		...
		if (mobLevel <= 4) {
			...
			this.species = World.Species.iceDragon;
	...
	public TextureRegion getAnimation() {
		switch (state) {
		case attack:
			return (TextureRegion) Assets.iceDragonAttack.getKeyFrame(stateTime, true);

		case walk:
			return (TextureRegion) Assets.iceDragonWalk.getKeyFrame(stateTime, true);

		case hit:
			return (TextureRegion) Assets.iceDragonHit.getKeyFrame(stateTime, true);

		case die:
			return (TextureRegion) Assets.iceDragonDie.getKeyFrame(stateTime, false);
		
		case read:
			return (TextureRegion) Assets.iceDragonSleep.getKeyFrame(stateTime, true);
		}
		return null;
	}
