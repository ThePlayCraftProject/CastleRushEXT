...
/** @author MrMarvel [S30] & friend's idea **/
public class WorldRenderer {
	...
	public WorldRenderer(World world, final CastleRush gam, Player player,
                         Table table) {
		this.world = world;
		game = gam;
		Assets.load(game);
	...
	private void renderSpells(Array<Spells> spells) {
		for (Spells i : spells) {
			try {
				...
				case fireBolt:
					if (!i.isHitTarget()) {
						keyFrame = (TextureRegion) Assets.fireBolt.getKeyFrame(
								i.getStateTime(), true);
						game.batch.draw(keyFrame, i.position.x, i.position.y,
								25, 25);

						keyFrame = (TextureRegion) Assets.sparklets.getKeyFrame(
								i.getStateTime(), true);
				...
				case grumblingIce:
					if (!i.isHitTarget()) {
						keyFrame = (TextureRegion) Assets.grumblingIce.getKeyFrame(
								i.getStateTime(), true);
				...
				System.out
						.println("Indexes for Spells ran out of exceptions!! Oh no!!");
			}
		}
	}
