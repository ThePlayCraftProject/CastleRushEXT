package marvel.android.castleattackers.game.try2.spells;


import com.badlogic.gdx.Gdx;
import marvel.android.castleattackers.game.try2.castleattackers.DynamicGameObject;
...
public class SpellEffect extends Spells {
	...
	protected DynamicGameObject.Condition condition;
	...
	public void init(){ // Is called, when the object is created/the spelleffect applies
		init = true;
		if(condition != DynamicGameObject.Condition.root)
	...
	protected void updateTickTimer(){
		...
		if(tickTimer <= 0){
			...
			if(effectDuration <= 0 || target.state == States.die && target != null ){
				if(target.condition.contains(DynamicGameObject.Condition.root, true))
	 ...
	 */
	public DynamicGameObject.Condition getCondition() {
		return condition;
	}
	 ...
	 */
	public void setCondition(DynamicGameObject.Condition condition) {
